...
function Model:load_obj()
    ...
    for line in love.filesystem.lines(self.obj_path) do
        ...
        if args[1] == "v" then
            ...
            table.insert(vertices, vertex_data)
        end
        ...
        if args[1] == "f" then
            ...
            -- split vertex data into 2 if it's 4 (we can only render trianlges)
            -- in the form: 1234 -> 123, 134
            for _, i in ipairs({1, 2, 3, 1, 4, 3}) do
                table.insert(indices, vert_indices[i])
            end
...
function Model:update(dt)
    ...
    self.view = mmath.mat4_lookAt({0, 0, 4}, {0, 0, 0}, {0, 1, 0})
    self.proj = mmath.mat4_perspective(math.rad(60), aspect, 0.1, 100)
end
